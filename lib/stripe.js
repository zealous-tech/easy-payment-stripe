...
class STRIPE extends BaseGateway {
  ...
  _createPaymentLink = async (order, stripeAccount) => {
    const payload = (({ customer, currency, orderId, description, amount, success_url, cancel_url, metadata }) => ({ customer, currency, orderId, description, amount, success_url, cancel_url, metadata   }))(order);
    try {
      const data = await this.client.checkout.sessions.create(
        {
        customer: payload.customer,
        mode: 'payment',
        line_items: [
  ...
  registerOrder = async (order) => {
    ...
    const orderPayload = (({ amount, currency, setup_future_usage, automatic_payment_methods, description, payment_method_types }) => ({ amount, currency, setup_future_usage, automatic_payment_methods, description, payment_method_types }))(order);
    orderPayload.customer = customerForConnectedAccount.data.id;
    const paymentIntent = await this._payOrder(orderPayload, payload.stripeAccount);
    paymentIntent.customerForConnectedAccount = customerForConnectedAccount.data.id;
    if (paymentIntent.hasError) {
  ...
  createPaymentLink = async (order) => {
    const payload = (({ stripeAccount, customer, customerData, connectedAccountCustomer }) => ({ stripeAccount, customer, customerData, connectedAccountCustomer }))(order);
    let customer;
    if (payload.customer) {
      customer = await this._retrieveCustomer(payload.customer);
      if (customer.hasError || customer.data.deleted) {
    ...
    let customerForConnectedAccount;
    if (customer && payload.connectedAccountCustomer) {
      customerForConnectedAccount = await this._retrieveConnectedAccountCustomer(payload.connectedAccountCustomer, payload.stripeAccount);
      if (customerForConnectedAccount.hasError || customerForConnectedAccount.data.deleted) {
        customerForConnectedAccount.hasError = true;
        customerForConnectedAccount.errorStep = 'customers.retrieve';
    ...
    const orderPayload = (({ stripeAccount, orderId, description, amount, currency, success_url, cancel_url, metadata }) => ({ stripeAccount, orderId, description, amount, currency, success_url, cancel_url, metadata }))(order);
    orderPayload.customer = customerForConnectedAccount.data.id;
    const data = await this._createPaymentLink(orderPayload, payload.stripeAccount);
    data.connectedAccountCustomer = customerForConnectedAccount.data.id;
    data.customer = customer.data.id;
    if (data.hasError) {
      data.errorStep = 'checkout.sessions.create';
    }
    return data;
  }
}
