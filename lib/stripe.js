...
class STRIPE extends BaseGateway {
  ...
  _capturePaymentFromStripeAccount = async (paymentIntentId, stripeAccount) => {
    try {
      const data = await this.client.paymentIntents.capture(paymentIntentId, { stripeAccount });
      return { hasError: false, data: data };
    } catch (err) {
      return { hasError: true, err: err };
    }
  ...
  capturePayment = async (order) => {
    const payload = (({ stripeAccount, paymentIntentId }) => ({ stripeAccount, paymentIntentId }))(order);
    const data = await this._capturePaymentFromStripeAccount(payload.paymentIntentId, payload.stripeAccount);
    if (data.hasError) {
